export function generarFacturaHTML(data: {
  nombre: string;
  telefono: string;
  email: string;
  fecha: string;
  ...
  servicios?: { nombre: string; descripcion?: string; precio?: number }[];
  ...
  <head>
    ...
    <style>
      ...
      .tabla-servicios tr:nth-child(even) td {
        background: #f7f7f7;
      }
      .tabla-servicios tr:nth-child(odd) td {
        background: #fff;
      }
      .tabla-servicios td:last-child, .tabla-servicios th:last-child {
        text-align: right;
      }
      .totales {
        width: 100%;
    ...
    <div class="factura-a4">
      ...
      <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: flex-start; margin-bottom: 32px;">
        ...
        <div style="flex: 1; text-align: right; min-width: 320px;">
          <div style="font-size: 44px; font-weight: 700; color: #aaa; letter-spacing: 4px; margin-bottom: 12px;">FACTURA</div>
          <table style="width: 100%; max-width: 340px; margin-left: auto; border-collapse: separate; border-spacing: 0; font-size: 15px; background: none; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
            ...
            <tr>
              <td style="padding: 7px 12px; color: #444; text-align: right; border-bottom: 1px solid #e0e0e0; border-top: 1px solid #e0e0e0; background: #fafafa;">N.º DE FACTURA:</td>
              <td style="padding: 7px 12px; color: #222; text-align: left; border-bottom: 1px solid #e0e0e0; border-top: 1px solid #e0e0e0; background: #fff;">${numeroFactura}</td>
            </tr>
            <tr>
          ...
          ${
            data.servicios && data.servicios.length > 0
              ? data.servicios
                  .map(
                    (servicio: any) =>
                      `<tr><td>${servicio.nombre === "Otros" ? servicio.descripcion : servicio.nombre}</td><td>${servicio.precio?.toFixed(2) ?? "0.00"} €</td></tr>`
                  )
                  .join("")
              : items
                  .map(
                    (item) =>
                      `<tr><td>${item.descripcion}</td><td>${(item.cantidad * item.precioUnit).toFixed(2)} €</td></tr>`
                  )
                  .join("")
          }
        </tbody>
      ...
      <div class="totales">
        <table class="totales-tabla">
          <tr><td>Total sin IVA</td><td>${subtotal.toFixed(2)} €</td></tr>
